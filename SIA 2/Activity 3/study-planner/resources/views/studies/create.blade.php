...
<div class="max-w-3xl mx-auto">
    <div class="bg-slate-900/40 rounded-3xl border border-slate-800 shadow-2xl overflow-hidden">
        ...
        <div class="px-8 py-6 bg-slate-800/50 border-b border-slate-800">
            ...
            <p class="text-slate-400 mt-1 ml-11">Fill in the details below to set up your new task.</p>
        </div>
        ...
        <div class="p-8">
            ...
            <form method="POST" action="{{ route('studies.store') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                {{-- Preview Area --}}
                <div id="preview-container" class="hidden animate-in zoom-in-95 duration-300">
                    <label class="block text-sm font-semibold text-slate-300 mb-2 ml-1">Image Preview</label>
                    <div class="relative inline-block">
                        <img id="preview" src="#" class="max-w-[200px] rounded-xl ring-4 ring-slate-800 shadow-2xl">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="notes" rows="4"
                        class="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-slate-200 placeholder-slate-600"
                        placeholder="Any additional details or links...">{{ old('notes') }}</textarea>
                </div>

                {{-- Actions --}}
                <div class="flex items-center gap-4 pt-4 border-t border-slate-800">
                    <button type="submit" class="flex-1 px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-2xl shadow-xl shadow-indigo-500/20 transition-all hover:-translate-y-0.5 active:translate-y-0 flex items-center justify-center gap-2">
